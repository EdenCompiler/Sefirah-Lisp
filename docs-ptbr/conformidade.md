...
| Símbolos | ◐ | `NIL` possui sua identidade de símbolo externo de `COMMON-LISP`. Os contratos testados incluem consultas, células separadas de valor/função, accessors graváveis, unbinding e property lists persistentes por `SYMBOL-PLIST`, `GET` e `REMPROP`; ainda faltam criação/cópia de símbolos e gensyms. |
