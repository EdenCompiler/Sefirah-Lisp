...
| Packages | ◐ | Packages no heap, opções de símbolo em fases no `DEFPACKAGE`, topologia reversível e consultas de uso/exportação, importação/uninterning, shadowing explícito e shadowing-import, consulta exata e seu estado, símbolos qualificados, adoção de símbolos não internados, proteção contra conflitos e persistência funcionam; faltam apelidos, remoção de packages e o restante da família ANSI. |
