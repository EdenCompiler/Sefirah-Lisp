...
| Packages | ◐ | Packages no heap, ciclo persistente de apelidos/renomeação/remoção, opções de símbolo em fases no `DEFPACKAGE`, topologia reversível e consultas de uso/exportação, importação/uninterning, shadowing explícito e shadowing-import, enumeração pela família `DO-SYMBOLS`, `FIND-ALL-SYMBOLS`, consulta exata e seu estado, símbolos qualificados, adoção de símbolos não internados, proteção contra conflitos e persistência funcionam; falta o restante da família ANSI. |
...
| Condições | ◐ | Objetos de condição, `ERROR`, `SIGNAL`, `HANDLER-BIND` com escopo dinâmico, `HANDLER-CASE`, `IGNORE-ERRORS`, objetos restart de primeira classe, descoberta/invocação dinâmica, cinco auxiliares padrão de restart nomeado e um seletor desktop suspensível para restarts sem argumentos funcionam; faltam associação por condição, hierarquia padrão completa, recuperação interativa com argumentos e opções completas de restart. |
