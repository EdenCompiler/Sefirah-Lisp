...
| Packages | ◐ | Packages no heap, ciclo persistente de apelidos/renomeação/remoção, opções de símbolo em fases no `DEFPACKAGE`, topologia reversível e consultas de uso/exportação, importação/uninterning, shadowing explícito e shadowing-import, enumeração pela família `DO-SYMBOLS`, consulta exata e seu estado, símbolos qualificados, adoção de símbolos não internados, proteção contra conflitos e persistência funcionam; falta o restante da família ANSI. |
