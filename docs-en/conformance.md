...
| Packages | ◐ | Heap packages, persistent nicknames and renaming, phased `DEFPACKAGE` symbol options, reversible use/export topology and inquiries, import/unintern, explicit shadowing and shadowing-import, exact lookup status, qualified symbols, uninterned-symbol adoption, conflict protection, and image persistence work; package deletion and the remaining ANSI package family are missing. |
